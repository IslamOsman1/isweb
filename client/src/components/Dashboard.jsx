...
function DashboardInner() {
  ...
  {!isOnlineDb && <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-2xl p-4 mb-6 flex items-start gap-3"><WifiOff className="text-yellow-400 flex-shrink-0 mt-0.5" size={20} /><div><p className="text-yellow-300 font-bold">{lang === 'ar' ? '⚠️ تحذير: قاعدة البيانات غير متصلة' : '⚠️ Warning: Database not connected'}</p><p className="text-yellow-200 text-sm mt-1">{lang === 'ar' ? 'التعديلات محفوظة محليًا فقط. تأكد من تشغيل الخادم عبر npm run dev:full حتى تظهر التعديلات على جميع الأجهزة.' : 'Changes are saved locally only. Make sure the server is running with npm run dev:full so changes appear on all devices.'}</p></div></div>}
